...
- **OpenLibrary cover fetch** — Every path that creates a `type='book'` resource calls `findBookCover()` (`lib/server/book-covers.ts`): it asks OpenLibrary for several matches and takes the first that actually carries a cover, then retries on title alone if an author narrowed the search to nothing. Lookups are best-effort — a miss or a network failure logs and stores nothing, and never blocks the save. `tests/book-covers.test.ts` fails the build if a new book-creating INSERT skips it.
