...
- **Cover fetch** — Every path that creates a `type='book'` resource calls `findBookCover()` (`lib/server/book-covers.ts`). It tries, in order: OpenLibrary with the title and author, then the title alone, then both again with the title *simplified* — subtitle after a colon and parenthetical series notes stripped. That last step matters more than it sounds: OpenLibrary indexes the main title while booklists record the title as printed, so `"Brothers in Hope: The Story of the Lost Boys of Sudan"` matches nothing while `"Brothers in Hope"` returns covers. When OpenLibrary has nothing and `GOOGLE_BOOKS_API_KEY` is set, Google Books is tried last (better coverage for children's and educational titles); without the key that step is skipped, because keyless Google requests share a permanently exhausted quota and return 429. Lookups are best-effort — a miss or a network failure logs and stores nothing, and never blocks the save. `tests/book-covers.test.ts` fails the build if a new book-creating INSERT skips it.
- **Never widen the cover search** — both providers are queried by *title* (`title=`, `intitle:`), never as a loose keyword search. A general OpenLibrary search for "Animals of the Sahara" returns the cover of "Le petit prince"; a wrong cover attached silently is worse than a missing one, which at least looks wrong.
- **Cover backfill and refresh** — Admin has a "Book covers" card that fills in every book missing one, pacing itself at roughly a request per second and reporting what it could not match. A book's own page has a "Refresh cover" button for the single-book case (usually a mistyped author); a failed refresh leaves the existing cover alone.
- **Attaching to a lesson** — `AddToLessonPicker` (`components/lessons/`) is the only add flow, reachable from the board's card view, each row of the course list view, and both lesson forms. It presents two groups matching the domain split: **lesson cards** (text, checklist, link/video, photo upload) write to `lesson_cards`; **materials** (book, supplies) write to `lesson_resources` so the planner's materials panel counts them. A book writes to both — a cover card on the board and a materials row. The retired FileRun option is gone from every surface.
...
4. **Import** — creates curriculum, bulk-creates lessons via `bulkCreateLessons`, then creates lesson cards via `bulkCreateLessonCards` (with YouTube oEmbed metadata fetch and `download_url` support for downloading Trello-hosted images locally via `downloadTrelloFile`), and attaches non-YouTube resources
